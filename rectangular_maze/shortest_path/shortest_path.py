...
def colourCell(img,row,column,colourVal):
    ...
    left = col_new - (cell // 2) + 1
    right = col_new + (cell // 2) - 2
    img[top + 1 : bottom, left + 1 : right] = colourVal
    for i in neighbours:
        if i[0] == row + 1:
            img[top + 1 : bottom + 2, left + 1 : right] = colourVal
        if i[0] == row - 1:
            img[top - 1 : bottom, left + 1 : right] = colourVal
        if i[1] == column + 1:
            img[top + 1 : bottom, left + 1 : right + 2] = colourVal
        if i[1] == column - 1:
            img[top + 1 : bottom, left - 1 : right] = colourVal
    ###################################################
    return img
...
def main(filePath, flag = 0):                 
    ...
    for i in shortestPath:         ## Loop to paint the solution path.
        img = colourCell(img, i[0], i[1], 100)
...
if __name__ == '__main__':
    filePath = 'maze02.jpg'        ## File path for test image
